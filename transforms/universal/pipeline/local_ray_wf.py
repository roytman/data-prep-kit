...
def doc_id():
    import subprocess
    import sys
    subprocess.run([sys.executable, "doc_id_local_ray.py"], check=True)

@dsl.component(base_image="quay.io/dataprep1/data-prep-kit/ededup-ray:0.2.0")
def ededup():
    import subprocess
    import sys
    subprocess.run([sys.executable, "ededup_local_ray.py"], check=True)

@dsl.component(base_image="quay.io/dataprep1/data-prep-kit/fdedup-ray:0.2.0")
def fdedup():
    import subprocess
    import sys
    subprocess.run([sys.executable, "fdedup_local_ray.py"], check=True)
